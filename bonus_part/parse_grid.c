...
void	parse_grid(char **array, int i, int y)
{
	if (array[i][y] == 'C')
		...
		array[i][y] = 'x';
	if (i - 1 > 0 && ft_strchr("CE0X", array[i - 1][y]))
		parse_grid(array, i - 1, y);
	if (y - 1 > 0 && ft_strchr("CE0X", array[i][y - 1]))
		parse_grid(array, i, y - 1);
	if (ft_strchr("CE0X", array[i][y + 1]))
		parse_grid(array, i, y + 1);
	if (array[i + 1] && ft_strchr("CE0X", array[i + 1][y]))
		parse_grid(array, i + 1, y);
}
